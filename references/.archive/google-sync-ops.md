...
```bash
HOME=/root AGENT_ROOT=<hermes-root> python3 <hermes-root>/skills/ocas-weave/scripts/google_sync.py
```

## Auth

- **Token path:** `/root/.google_workspace_mcp/credentials/owner.operator.json` (symlinked from the path the script expects)
- **TOKEN_PATH:** `google-workspace-user.json` ONLY — never silently fall back to Indigo's token
...
- **Extension path:** `LADYBUG_EXTENSION_PATH=<hermes-home>/home/.lbdb/extension/0.17.0/linux_amd64`
