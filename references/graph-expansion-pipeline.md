...
**Key pitfall: email-domain org inference fails for career transitions.** `contact@example.com` → org inference says "Google" but person is at Sportradar. `contact@example.com` → org inference says "Unknown" but person is at Google (googlemail.com = consumer Gmail alias used by Googlers). Always cross-check email-domain inference against Sift deep dive findings before classifying employers.
...
results = delegate_task(
    ...
    toolsets=['web'],
    context="Yury Pinsky is Director of Product Management at Google, leading Gemini/Bard extensions. Email: contact@example.com"
)
```
...
```jsonl
{"name": "Onny Chatterjee", "email": "contact@example.com", "id": "17dbb3f4-344a-41e5-8e54-e768f965e4a8"}
{"name": "Ankita Akerkar", "email": "contact@example.com", "id": "6b3053ba-ec9c-513a-91c1-9483a60cb819"}
```
...
```python
expansion_dir = Path('<hermes-root>/commons/data/ocas-expansion')
...
- Google Docs API: Requires user OAuth token at `<hermes-root>/google_token.json` (NOT service account). Token successfully created docs even without explicit `documents` scope in April 2026 run. **If OAuth flow is not completed (no `google_token.json` exists), skip Google Docs and save locally.**
...
- **Local report fallback** — When Google Drive quota exceeded, save markdown to `<hermes-root>/commons/data/ocas-expansion/report_YYYY-MM-DD.md` and write status to `last_run_report.txt`
...
9. Use cached data from previous runs when available (check `<hermes-root>/commons/data/ocas-expansion/`)
...
db = lb.Database('<hermes-root>/commons/db/ocas-weave/weave.lbug')
...
|---|---|---|---|
| `contact@example.com` (long handle) | 100 | deliverable | ✓ Valid |
| `contact@example.com` (short handle) | 88 | deliverable | ✓ Valid |
| `contact@example.com` (short handle) | 88 | deliverable | ✓ Valid |
| `contact@example.com` (short handle) | 89 | deliverable | ✓ Valid |
| `contact@example.com` (short handle) | 91 | deliverable | ✓ Valid |
| `contact@example.com` (short handle) | 0 | undeliverable | ✗ False negative |
| `contact@example.com` (short handle) | 0 | undeliverable | ✗ False negative |
| `contact@example.com` (long handle) | 73 | risky | ⚠ Valid but risky |
| `contact@example.com` (personal) | 88 | deliverable | ✓ Valid |
| `contact@example.com` (alias) | 91 | deliverable | ✓ Valid |
...
html_file = f'<hermes-root>/commons/data/ocas-expansion/{run_id}-report.html'
...
**`@googlemail.com` is a Gmail alias used by Googlers (April 2026 finding):** Some Google employees use `@googlemail.com` addresses instead of `@google.com`. Domain inference treats these as "Unknown" since it's technically a consumer Gmail domain, but the person is at Google. Marc Paulina (`contact@example.com`) is an EMEA UX Lead at Google. Always treat `@googlemail.com` the same as `@google.com` for employer inference.

**Hunter.io email verification of short-handle @google.com addresses (April 2026 finding):**

Hunter.io's email-verifier endpoint returns `score=0` and `result=undeliverable` for short-handle Google email addresses (e.g., `contact@example.com`, `contact@example.com`). This is a false negative — these addresses are valid Google employee emails confirmed by Google Developer profile probes (returning HTTP 200). Longer handles verify correctly (e.g., `contact@example.com` → score=100, `contact@example.com` → score=88). The likely cause is Google's SMTP server rejecting verification probes for short aliases. When Hunter.io email verification returns `undeliverable` for an `@google.com` address, cross-reference with Google Developer profile probing (`developers.google.com/profile/u/{handle}`) before concluding the email is invalid. If the dev profile returns 200, the email is valid despite the Hunter score.
...
report_file = "<hermes-root>/commons/data/ocas-expansion/expansion_report.md"
...
# Save link
with open("<hermes-root>/commons/data/ocas-expansion/last_run_report.txt", "w") as f:
    f.write(drive_link)
```
...
with open('<hermes-root>/commons/data/ocas-expansion/run_report.md', 'r') as f:
    report_md = f.read()
...
- Token at `<hermes-root>/google_token.json` was valid and not expired
...
try:
    ...
    report_path = '<hermes-root>/commons/data/ocas-expansion/graph_expansion_report.md'
...
- `<hermes-root>/commons/data/ocas-expansion/scout_findings.json` — Scout research results (confidence-scored)
- `<hermes-root>/commons/data/ocas-expansion/sift_findings.json` — Deep dive findings
- `<hermes-root>/commons/data/ocas-expansion/complete_dossier.json` — Merged Scout + Sift data
- `<hermes-root>/commons/data/ocas-expansion/final_weave_synthesis.json` — Synthesis with classification flags
- `<hermes-root>/commons/data/ocas-expansion/graph_expansion_report.md` — Full markdown report
- `<hermes-root>/commons/data/ocas-expansion/last_run_report.txt` — Google Doc link or local path
...
| Google Docs service account auth fails | Service account credentials don't work for Docs API. Use user OAuth token at `<hermes-root>/google_token.json` instead. |
...
**Google Docs API unavailable** — Fall back to local markdown report. Save to `<hermes-root>/commons/data/ocas-expansion/graph_expansion_report.md` and write the path to `last_run_report.txt`.

**Google Docs creation may fail with `ACCESS_TOKEN_SCOPE_INSUFFICIENT` (April 2026 cron finding)** — Even when the token at `<hermes-root>/google_token.json` exists, `documents.create()` can return HTTP 403 with `ACCESS_TOKEN_SCOPE_INSUFFICIENT`. This happens when the token's scopes don't include `https://www.googleapis.com/auth/documents`. The `drive.file` scope alone is NOT always sufficient — it depends on how the token was created. When this error occurs, fall back immediately to `rclone copy` (which uploads the markdown file to Google Drive) or local file saves.
...
| Google Docs service account read-only | OCIGCP service account can read existing docs (`documents.get`) but cannot create (`documents.create` → 403) or write (`batchUpdate` → 403). Use user OAuth token at `<hermes-root>/google_token.json` for write access, or fall back to local markdown. |
| Google Docs `ACCESS_TOKEN_SCOPE_INSUFFICIENT` | Token at `<hermes-root>/google_token.json` may lack `documents` scope even if `drive.file` is present. Fall back to `rclone copy {report_file} GDrive:/hermes-reports/` for upload, or save locally and write `file://` path to `last_run_report.txt`. |
