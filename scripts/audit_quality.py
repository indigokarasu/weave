...
def _script_dir():
    return os.path.join(_prof(), "skills", "ocas-weave", "scripts")


def _db_path():
    return os.path.join(_prof(), "commons", "db", "ocas-weave", "weave.sqlite")
...
def main():
    ...
    from url_quality import is_person_profile, handle_is_opaque  # noqa: E402
    ...
    for f in facts:
        ...
        if not is_person_profile(f["value"], per.get("name") or "",
                                 plat, names,
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="audit_quality.py — Sweep the store for data-quality defects.")
    ap.parse_args()
    main()
