...
- **Paste-back OAuth flow**: The listener timeout must be ≥ 15 minutes for user interaction. The `oauth_complete_v2.py` script at `<hermes-root>/scripts/oauth_complete_v2.py` has a 15-minute timeout and writes the code to `/tmp/oauth_code.txt` immediately on receipt. If the process dies, the code is still in that file.
...
fuser <hermes-root>/commons/db/ocas-weave/weave.lbug  # should return empty

# 4. Remove stale WAL
rm -f <hermes-root>/commons/db/ocas-weave/weave.lbug.wal

# 5. Run the sync/script
cd <hermes-root> && HOME=/root AGENT_ROOT=<hermes-root> python3 <script>
