...
def get_contacts_needing_enrichment():
    ...
    _all = 1 if os.environ.get("WEAVE_ENRICH_ALL") == "1" else 0
    rows = weave.execute("""
        ...
        WHERE name IS NOT NULL AND name != ''
          -- Same exclusions the contact-create path applies, and for the same
          -- reasons: a pseudo row is a relation rather than a person, an
          -- archived contact was deliberately retired, and a deceased contact
          -- must never be researched.
          ...
          --   * confidence >= 0.9 is self-assigned -- 267 of those rows came
          --     from a bulk import and 25 from the pipeline itself, and 105 of
          --     them still have empty fields.
          --   * "no gaps" only means some earlier version already filled them,
          --     and those versions wrote payload blobs, name-matched strangers
          --     and uncorroborated employers.
          -- The pseudo/archived/deceased exclusions above are NOT overridable
          -- here; they are about who may be researched, not about data quality.
          AND (? = 1 OR (source_type IS NULL OR (source_type != 'user_provided' AND source_type != 'user_correction')))
          AND (? = 1 OR (confidence IS NULL OR confidence < 0.9))
          AND (? = 1 OR ((org IS NULL OR org = '')
    ...
    for row in rows:
        ...
        # A record with no empty field has nothing to FILL, but its values may
        # still have come from a version that fabricated them. In enrich-all mode
        # every field is re-verified instead; the write path is fill-only, so
        # this adds freshly-gated facts to the graph rather than overwriting the
        # record.
        if _all and not contact["gaps"]:
            contact["gaps"] = ["org", "occupation", "location_city", "email", "phone"]
...
def recalculate_enrichability_sqlite(weave, contact_id):
    ...
    # Delete the old score -- EDGES FIRST, then the facts they point at.
    #
    # Deleting the fact alone leaves its HasFact edge pointing at a row that no
    # longer exists: one stranded edge per contact per run, which is how 25,585
    # of them accumulated (65.6% of every HasFact edge in the store). The same
    # defect was fixed in recalculate_enrichability.py, but this file carries its
    # own second copy of the scoring logic and was missed, so the leak continued
    # from here. Any future change to one copy has to be made to the other; they
    # should really be one function.
    _stale = [r["id"] for r in weave.execute("""
    ...
    """, (contact_id,))]
    for _fid in _stale:
        weave.execute("DELETE FROM edges WHERE target_id = ? AND rel_type = 'HasFact'",
                      (_fid,))
        weave.execute("DELETE FROM facts WHERE id = ?", (_fid,))
...
# Deliberately NOT /var/lock/weave-enrichment.lock: the crontab wraps the
# nightly run in flock on that path, so taking it again from inside the
# process would make the cron job block on itself and exit immediately.
# Separate path -> the crontab guard stops cron overlapping cron, and this
# one stops every other combination.
LOCK_PATH = "/var/lock/weave-enrichment-pipeline.lock"


def acquire_pipeline_lock():
    """Exclusive, non-blocking. Returns the held file object, or None.

    The handle must stay referenced for the life of the run: if it is garbage
    collected the OS releases the lock and the guard silently stops guarding.
    """
    import fcntl
    try:
        fh = open(LOCK_PATH, "a+")
...
def main():
    ...
    if _lock is None:
        log("Another enrichment run holds the lock; exiting rather than "
            "racing it (concurrent runs double-write facts).")
        return
    globals()["_PIPELINE_LOCK"] = _lock      # keep it held for the whole run
    ...
    _ignore_cd = os.environ.get("WEAVE_IGNORE_COOLDOWN") == "1"
    ...
    def off_cooldown(c):
        ...
        # An attempt recorded by an older pipeline is not evidence about what
        # the CURRENT pipeline would do, whichever way it went. A failure then
        # does not mean this person cannot be found now. And a SUCCESS then is
        # not a reason to wait 90 days, because the versions that produced those
        # successes were broken: they wrote whole serialized payloads into single
        # fact values, attributed strangers' profiles to contacts by name alone,
        # and put employers on records with nothing corroborating them. Data from
